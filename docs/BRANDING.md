...
- Subheadline pairs the "See what drives sales…" promise with time-bound offers such as "Sign up now to unlock lifetime access for free." Keep it to two sentences with 16–18px text and no semicolons or em dashes. The lifetime access sentence is styled bold with `font-semibold` and uses primary text colors (`text-gray-900 dark:text-gray-100`) to emphasize the offer.
- CTA button beneath the hero displays "How to Export Your Klaviyo Data" (replacing the earlier "Report Export Guide" label) and links to `/report-export-guide` with an external link icon.
...
- Beneath the testimonial, show the four-proof stack: each screenshot card is full width, stacked vertically with `rounded-3xl border border-gray-200/70 dark:border-gray-700/70 bg-white/90 dark:bg-gray-900/70 shadow-[0_10px_30px_rgba(15,23,42,0.08)]`. The benefit header now sits outside the bordered container, centered above it in `text-xl md:text-2xl font-semibold`, followed by the bordered image with `px-6 py-6` gutters and a rounded screenshot. Images live at `app/homepage_images` and appear in this order: Email Performance, Flow Step Analysis, Inactivity Revenue, Send Volume Impact.
