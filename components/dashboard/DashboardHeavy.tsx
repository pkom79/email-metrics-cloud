...
export default function DashboardHeavy({ businessName, userId }: { businessName?: string; userId?: string }) {
    ...
    // PoP
    const calcPoP = useMemo(() => { const negative = new Set(['unsubscribeRate', 'spamRate', 'bounceRate']); const pick = (k: string, a: any) => { switch (k) { case 'totalRevenue': return a.totalRevenue; case 'avgOrderValue': case 'averageOrderValue': return a.avgOrderValue; case 'revenuePerEmail': return a.revenuePerEmail; case 'openRate': return a.openRate; case 'clickRate': return a.clickRate; case 'clickToOpenRate': return a.clickToOpenRate; case 'emailsSent': return a.emailsSent; case 'totalOrders': return a.totalOrders; case 'conversionRate': return a.conversionRate; case 'unsubscribeRate': return a.unsubscribeRate; case 'spamRate': return a.spamRate; case 'bounceRate': return a.bounceRate; default: return 0; } }; return (metricKey: string, dataset: 'all' | 'campaigns' | 'flows', options?: { flowName?: string }) => { const isAll = dateRange === 'all'; if (isAll) return { changePercent: 0, isPositive: true, previousValue: 0, previousPeriod: undefined as any }; let startDate: Date, endDate: Date, periodDays: number; if (dateRange === 'custom' && customActive) { startDate = new Date(customFrom! + 'T00:00:00'); endDate = new Date(customTo! + 'T23:59:59'); periodDays = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)); } else { periodDays = parseInt(String(dateRange).replace('d', '')); if (!Number.isFinite(periodDays) || periodDays <= 0) return { changePercent: 0, isPositive: true, previousValue: 0, previousPeriod: undefined as any }; endDate = new Date(REFERENCE_DATE); endDate.setHours(23, 59, 59, 999); startDate = new Date(endDate); startDate.setDate(startDate.getDate() - periodDays + 1); startDate.setHours(0, 0, 0, 0); } const prevEndDate = new Date(startDate); prevEndDate.setDate(prevEndDate.getDate() - 1); prevEndDate.setHours(23, 59, 59, 999); let prevStartDate: Date; if (periodDays === 1) { prevStartDate = new Date(prevEndDate); prevStartDate.setHours(0, 0, 0, 0); } else { prevStartDate = new Date(prevEndDate); prevStartDate.setDate(prevStartDate.getDate() - periodDays + 1); prevStartDate.setHours(0, 0, 0, 0); } let campaignsToUse = ALL_CAMPAIGNS; let flowsToUse = ALL_FLOWS; if (dataset === 'campaigns') flowsToUse = [] as any; if (dataset === 'flows') campaignsToUse = [] as any; const flowName = options?.flowName; if (dataset === 'flows' && flowName && flowName !== 'all') { flowsToUse = flowsToUse.filter(f => f.flowName === flowName); } const current = dm.getAggregatedMetricsForPeriod(campaignsToUse as any, flowsToUse as any, startDate, endDate); const previous = dm.getAggregatedMetricsForPeriod(campaignsToUse as any, flowsToUse as any, prevStartDate, prevEndDate); const currentValue = pick(metricKey, current); const previousValue = pick(metricKey, previous); let changePercent = 0; if (previousValue !== 0) changePercent = ((currentValue - previousValue) / previousValue) * 100; else if (currentValue > 0) changePercent = 100; const isPositive = negative.has(metricKey) ? changePercent < 0 : changePercent > 0; return { changePercent, isPositive, previousValue, previousPeriod: { startDate: prevStartDate, endDate: prevEndDate } }; }; }, [dateRange, dm, customActive, customFrom, customTo, REFERENCE_DATE, ALL_CAMPAIGNS, ALL_FLOWS]);
