...
export default function DashboardHeavy({ businessName, userId }: { businessName?: string; userId?: string }) {
    ...
    const formatMetricValue = (v: number, metric: string) => {
        if (['revenue', 'avgOrderValue', 'revenuePerEmail'].includes(metric)) {
            // Show cents for small values; keep compact style for larger numbers
            return v < 1000 ? formatCurrency(v) : formatCompactCurrency(v);
        }
            ...
            <div className="p-6"><div className="max-w-7xl mx-auto space-y-8">
                {overviewMetrics && (
                    <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            ...
                            <MetricCard title="Revenue per Email" value={(overviewMetrics.revenuePerEmail.value < 1000 ? formatCurrency(overviewMetrics.revenuePerEmail.value) : formatCompactCurrency(overviewMetrics.revenuePerEmail.value))} change={overviewMetrics.revenuePerEmail.change} isPositive={overviewMetrics.revenuePerEmail.isPositive} previousValue={overviewMetrics.revenuePerEmail.previousValue} previousPeriod={overviewMetrics.revenuePerEmail.previousPeriod} dateRange={dateRange} metricKey="revenuePerEmail" sparklineData={overviewSeries.revenuePerEmail} compareMode={compareMode} category="email" />
                            {/* Row 3 */}
                ...
                {campaignMetrics && (
                    <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {/* Row 1 */}
                            <MetricCard title="Total Revenue" value={(campaignMetrics.totalRevenue.value < 1000 ? formatCurrency(campaignMetrics.totalRevenue.value) : formatCompactCurrency(campaignMetrics.totalRevenue.value))} change={campaignMetrics.totalRevenue.change} isPositive={campaignMetrics.totalRevenue.isPositive} previousValue={campaignMetrics.totalRevenue.previousValue} previousPeriod={campaignMetrics.totalRevenue.previousPeriod} dateRange={dateRange} metricKey="revenue" sparklineData={campaignSeries.totalRevenue} compareMode={compareMode} category="campaign" />
                            <MetricCard title="Average Order Value" value={(campaignMetrics.averageOrderValue.value < 1000 ? formatCurrency(campaignMetrics.averageOrderValue.value) : formatCompactCurrency(campaignMetrics.averageOrderValue.value))} change={campaignMetrics.averageOrderValue.change} isPositive={campaignMetrics.averageOrderValue.isPositive} previousValue={campaignMetrics.averageOrderValue.previousValue} previousPeriod={campaignMetrics.averageOrderValue.previousPeriod} dateRange={dateRange} metricKey="avgOrderValue" sparklineData={campaignSeries.averageOrderValue} compareMode={compareMode} category="campaign" />
                            ...
                            <MetricCard title="Revenue per Email" value={(campaignMetrics.revenuePerEmail.value < 1000 ? formatCurrency(campaignMetrics.revenuePerEmail.value) : formatCompactCurrency(campaignMetrics.revenuePerEmail.value))} change={campaignMetrics.revenuePerEmail.change} isPositive={campaignMetrics.revenuePerEmail.isPositive} previousValue={campaignMetrics.revenuePerEmail.previousValue} previousPeriod={campaignMetrics.revenuePerEmail.previousPeriod} dateRange={dateRange} metricKey="revenuePerEmail" sparklineData={campaignSeries.revenuePerEmail} compareMode={compareMode} category="campaign" />
                            {/* Row 3 */}
