...
export default function DashboardHeavy({ businessName, userId }: { businessName?: string; userId?: string }) {
    ...
    // Flow Step Analysis now always visible (removed collapse/expand per updated UX request)
    ...
    return (
        <div className="min-h-screen relative">
            ...
            <div className="p-6"><div className="max-w-7xl mx-auto space-y-8">
                {overviewMetrics && (
                    <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {/* Row 1 */}
                            <MetricCard title="Total Revenue" value={formatCurrency(overviewMetrics.totalRevenue.value)} change={overviewMetrics.totalRevenue.change} isPositive={overviewMetrics.totalRevenue.isPositive} previousValue={overviewMetrics.totalRevenue.previousValue} previousPeriod={overviewMetrics.totalRevenue.previousPeriod} dateRange={dateRange} metricKey="revenue" sparklineData={overviewSeries.totalRevenue} compareMode={compareMode} segment="all" band={(() => { try { const combined = [...filteredCampaigns, ...filteredFlowEmails]; if (!combined.length) return null; const times = combined.map(e => e.sentDate.getTime()).filter(t => Number.isFinite(t)); if (!times.length) return null; const startDate = new Date(Math.min(...times)); startDate.setHours(0, 0, 0, 0); const endDate = new Date(Math.max(...times)); endDate.setHours(23, 59, 59, 999); const gran = dm.getGranularityForRange(startDate, endDate); return dm.getHistoricalBand(filteredCampaigns, filteredFlowEmails, 'revenue', startDate, endDate, gran); } catch { return null; } })()} />
                            ...
                            {/* Row 2 */}
                            <MetricCard title="Open Rate" value={formatPercent(overviewMetrics.openRate.value)} change={overviewMetrics.openRate.change} isPositive={overviewMetrics.openRate.isPositive} previousValue={overviewMetrics.openRate.previousValue} previousPeriod={overviewMetrics.openRate.previousPeriod} dateRange={dateRange} metricKey="openRate" sparklineData={overviewSeries.openRate} compareMode={compareMode} segment="all" band={(() => { try { const combined = [...filteredCampaigns, ...filteredFlowEmails]; if (!combined.length) return null; const times = combined.map(e => e.sentDate.getTime()).filter(t => Number.isFinite(t)); if (!times.length) return null; const startDate = new Date(Math.min(...times)); startDate.setHours(0, 0, 0, 0); const endDate = new Date(Math.max(...times)); endDate.setHours(23, 59, 59, 999); const gran = dm.getGranularityForRange(startDate, endDate); return dm.getHistoricalBand(filteredCampaigns, filteredFlowEmails, 'openRate', startDate, endDate, gran); } catch { return null; } })()} />
                ...
                {/* Flow Step Analysis (always visible) */}
                <section>
                    <div className="mb-8">
                        <h3 className="flex items-center gap-2 text-xl font-bold text-gray-900 dark:text-gray-100 mb-3"><Zap className="w-5 h-5 text-purple-600" />Flow Step Analysis</h3>
