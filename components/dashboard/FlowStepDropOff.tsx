...
type MetricKey = 'openRate' | 'clickRate' | 'conversionRate' | 'revenuePerEmail' | 'revenue';

// Standard US currency formatting with two decimals everywhere
...
const StepCell: React.FC<{
    ...
    const isCurrency = metric === 'revenuePerEmail' || metric === 'revenue';
    ...
    // Better text contrast: white for darker backgrounds, dark for light backgrounds
    const isDarkBackground = mag > 0.3; // If the background has significant color intensity
    const textColor = delta === 0 ? 'text-gray-900' : (isDarkBackground ? 'text-white' : 'text-gray-900');

    return (
        <div className="relative group rounded-lg px-3 py-3 font-semibold text-base leading-none select-none" style={{ background: bg }}>
            <span className={`tabular-nums ${textColor}`}>{displayValue}</span>
            <div className="pointer-events-none absolute left-1/2 bottom-full z-[100] hidden -translate-x-1/2 -translate-y-2 group-hover:block">
                <div className="w-64 rounded-xl border border-gray-200 bg-white shadow-2xl p-4 text-[11px] text-gray-800 backdrop-blur-sm">
                    ...
                    <table className="w-full text-[11px]">
                        <tbody>
                            ...
                            <tr><td className="text-gray-500 pr-2">Revenue</td><td className="text-right tabular-nums">{formatCurrency(onHoverData.revenue)}</td></tr>
                            <tr><td className="text-gray-500 pr-2">Rev/Email</td><td className="text-right tabular-nums">{formatCurrency(onHoverData.rpe)}</td></tr>
...
export default function FlowStepDropOff({ dateRange, customFrom, customTo }: Props) {
    ...
    }, [flowEmails, dateRange, customFrom, customTo]);
    ...
    const { byFlow, flows, maxSeq, presence } = useMemo(() => {
        ...
        for (const e of filtered) {
            ...
            if (!cell) {
                cell = { sequencePosition: e.sequencePosition, emailsSent: 0, opens: 0, clicks: 0, orders: 0, revenue: 0, revenuePerEmail: 0, openRate: 0, clickRate: 0, conversionRate: 0, deltaRevenue: 0, deltaRevenuePerEmail: 0, deltaOpenRate: 0, deltaClickRate: 0, deltaConversionRate: 0 };
                map[flow].push(cell);
            }
        ...
        Object.values(map).forEach(arr => {
            ...
            arr.forEach((c, i) => {
                ...
                c.deltaRevenue = c.revenue - prev.revenue;
    ...
    flows.forEach(f => {
        ...
        cells.forEach(c => {
            ...
            const delta = (metric === 'revenue' ? c.deltaRevenue : metric === 'revenuePerEmail' ? c.deltaRevenuePerEmail : metric === 'openRate' ? c.deltaOpenRate : metric === 'clickRate' ? c.deltaClickRate : c.deltaConversionRate);
    ...
    const metricOptions: { key: MetricKey; label: string; available: boolean }[] = [
        ...
        { key: 'revenue', label: 'Revenue', available: true },
        ...
        <div className="mt-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between mb-5">
                ...
                <div className="relative">
                    <SelectBase value={metric} onChange={e => setMetric((e.target as HTMLSelectElement).value as MetricKey)} className="px-3 h-9 pr-8 rounded-lg border bg-white border-gray-300 text-gray-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        {metricOptions.filter(o => o.available).map(o => <option key={o.key} value={o.key}>{o.label}</option>)}
                    </SelectBase>
                </div>
            </div>
            <div className="relative rounded-2xl overflow-hidden bg-white">
                {/* Right fade overlay - only show when scrollable */}
                                ...
                                return (
                                    <tr key={flow} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        {Array.from({ length: effectiveMax }, (_, i) => i + 1).map(step => {
                                            ...
                                            const deltaVal = step === 1 ? 0 : (metric === 'revenue' ? cell.deltaRevenue : metric === 'revenuePerEmail' ? cell.deltaRevenuePerEmail : metric === 'openRate' ? cell.deltaOpenRate : metric === 'clickRate' ? cell.deltaClickRate : cell.deltaConversionRate);
                                            return (
