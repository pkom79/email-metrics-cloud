...
export default function DashboardClient({ businessName, userId }: { businessName?: string; userId?: string }) {
    ...
    }, [dateRange, customActive, customDays, dm]); // dataVersion not needed: granularity derives only from inputs
    ...
    }, [dateRange, dm, customActive, customFrom, customTo, REFERENCE_DATE, ALL_CAMPAIGNS, ALL_FLOWS]); // removed unused: customDays, selectedFlow
    ...
    useEffect(() => {
        if (!hasData) return;
        if (dateRange === 'custom') return; // custom already drives inputs directly
    ...
    }, [defFlows, calcPoP, selectedFlow]); // include selectedFlow (passed into calcPoP options)
