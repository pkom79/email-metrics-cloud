-- Restore multi-account memberships with roles and align RLS/policies with is_account_member().
-- Adds account_users, role helpers, owner sync trigger, and updates storage/object policies.
...
-- Keep owner membership row in sync
...
-- Backfill owner membership rows
insert into public.account_users (account_id, user_id, role)
...
create policy account_users_insert_owner_or_admin on public.account_users
  for insert to authenticated with check (
    public.is_admin()
    or exists (
      select 1 from public.accounts a where a.id = account_id and a.owner_user_id = auth.uid()
    )
  );
...
create policy snapshot_totals_select_member_or_admin on public.snapshot_totals
  ...
  using (
    ...
    or exists (
      ...
      where s.id = snapshot_totals.snapshot_id
...
create policy snapshot_series_select_member_or_admin on public.snapshot_series
  ...
  using (
    ...
    or exists (
      select 1 from public.snapshots s
      where s.id = snapshot_series.snapshot_id
        and public.is_account_member(s.account_id)
    )
  );
...
-- Storage RLS: uploads + csv-uploads buckets
do $$
begin
  begin
