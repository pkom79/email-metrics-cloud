...
async function testFlowAnalyticsLive() {
    try {
        ...
        const response = await fetch(`${BASE_URL}/api/klaviyo/flow-analytics?format=csv&admin_secret=${KLAVIYO_API_KEY}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
...
async function findCanaryAccount() {
    ...
    accounts.forEach(acc => {
        console.log(`  - ID: ${acc.id}, Company: ${acc.company}`);
    });

    const canaryAccount = accounts.find(acc =>
        acc.company && acc.company.toLowerCase().includes('canary')
    );

    if (canaryAccount) {
        console.log(`✅ Found canary account: ${canaryAccount.id}`);
        return canaryAccount;
    } else {
        console.log('⚠️  No canary account found. Creating test account reference.');
        return { id: 'acc_canary_1', company: 'Canary Test Account' };
    }
}
...
Sample Campaign,Special Offer,2024-01-16T14:00:00Z,200,120,25,500.00`;
    ...
    try {
        ...
        const subscribersResponse = await fetch(uploadData.urls.subscribers.token, {
            method: 'PUT',
            body: subscribersData,
            headers: { 'Content-Type': 'text/csv' }
        });
        ...
        const campaignsResponse = await fetch(uploadData.urls.campaigns.token, {
            method: 'PUT',
            body: campaignsData,
            headers: { 'Content-Type': 'text/csv' }
        });
...
async function linkUploadToAccount(uploadId, accountId) {
    ...
    try {
        ...
        if (error) {
            console.error('❌ Upload linking failed:', error);
            return null;
        }
...
async function verifyIntegration(accountId, uploadId) {
    ...
    try {
        ...
        if (upload) {
            console.log(`📊 Upload ${upload.id}: ${upload.status} for account ${upload.account_id}`);
        }
        ...
        // Check storage files
        const bucket = process.env.DATA_INGEST_BUCKET || process.env.PREAUTH_BUCKET || 'preauth-uploads';
        const { data: files } = await supabase.storage
            .from(bucket)
            .list(uploadId, { limit: 10 });
...
async function main() {
    ...
    try {
        ...
        // Verify integration
        await verifyIntegration(canaryAccount.id, uploadData.uploadId);
